...
* `_guard_real_state` — ostatnia linia obrony: prawdziwy `config.toml`, `.env` i
  `out/ledger.json` muszą być po sesji nietknięte. Ledger jest źródłem prawdy dla
  numeracji faktur, więc jego przypadkowa modyfikacja to problem prawny, nie testowy.

Fabryki katalogów (`make_root`, `healthy_root`) siedzą w `tests/support.py`.
"""
...
# Pliki, których żaden test nie ma prawa ruszyć — w checkoucie i w docelowym home.
_GUARDED = (
    REPO_ROOT / "config.toml",
    REPO_ROOT / ".env",
    REPO_ROOT / "out" / "ledger.json",
    Path.home() / ".ksef-invoice" / "config.toml",
    Path.home() / ".ksef-invoice" / ".env",
    Path.home() / ".ksef-invoice" / "out" / "ledger.json",
)
